...
// Standalone P3-SP K-stage PageRank wavefront test.

module tb_iter_parallel_in_online_mma8_global_wavefront_top;
...
`endif
    ...
    localparam integer ONLINE_DELAY = 2;
...
`include "iter_tb_signed_digit_reconstruct.vh"
    ...
    iter_parallel_in_online_mma8_global_wavefront_top #(
    ...
    initial begin
        ...
        wait_count = 0;
        while (!w_final_done_rows[0] && wait_count < 256) begin
            @(posedge i_clk);
        ...
        if (!w_stage_done[NUM_STAGES - 1] || capture_count !== DATA_WIDTH) begin
            $display("ERROR parallel-in wavefront did not complete done=%b capture=%0d counts=%h",
                w_stage_done, capture_count, w_stage_valid_count);
            $fatal;
        end

        if (w_stage_started_before_prev_done !== {(NUM_STAGES - 1){1'b1}}) begin
            $display("ERROR expected direct stage overlap flags=%b", w_stage_started_before_prev_done);
            $fatal;
        end
        ...
        for (ci = 0; ci < NUM_CLUSTERS; ci = ci + 1) begin
            ...
            for (ri = 0; ri < ROWS_PER_CLUSTER; ri = ri + 1) begin
                ...
                if (abs_delta > VALUE_TOLERANCE) begin
                    $display("ERROR parallel-in state mismatch K=%0d row=%0d got=%0d expected=%0d p/n=%h/%h gold=%h/%h",
                        NUM_STAGES, global_row, dut_value, gold_value,
        ...
        $display("PASS tb_iter_parallel_in_online_mma8_global_wavefront_top");
        $display("COUNTERS parallel_in_wavefront K=%0d delay=%0d fast2=%0d total=%0d capture=%0d stage_counts=%h overlap=%b",
            NUM_STAGES, ONLINE_DELAY, FAST2_CORE, cycle_count, capture_count,
            w_stage_valid_count, w_stage_started_before_prev_done);
        $finish;
    end
