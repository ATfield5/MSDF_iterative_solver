...
module tb_online_row_update_delta_slice;
    ...
    online_row_update_delta_slice #(
        ...
        .i_x_old_p(i_x_old_p), .i_x_old_n(i_x_old_n),
        .i_tail_bound(i_tail_bound), .i_eps_d(i_eps_d),
        .o_valid(o_valid), .o_sum_p(o_sum_p), .o_sum_n(o_sum_n),
        .o_delta_word(o_delta_word),
        .o_abs_prefix(o_abs_prefix), .o_abs_upper(o_abs_upper), .o_abs_lower(o_abs_lower),
        .o_cert_converged(o_cert_converged), .o_cert_not_converged(o_cert_not_converged),
        ...
        repeat (6) begin
            ...
            if (o_valid) begin
                ...
                if ((^o_sum_p === 1'bx) || (^o_sum_n === 1'bx)) begin
                    $display("ERROR tb_online_row_update_delta_slice unknown row-update output");
                    $fatal;
                end
                if (o_delta_word !== 13'sd11 ||
                    o_abs_prefix !== 13'd11 ||
                    o_abs_upper !== 13'd12 ||
                    o_abs_lower !== 13'd10 ||
                    !o_cert_converged ||
                    o_cert_not_converged ||
                    o_cert_state !== 2'b01) begin
                    $display("ERROR tb_online_row_update_delta_slice delta=%0d abs=%0d up=%0d low=%0d conv=%b not=%b state=%b",
                        o_delta_word, o_abs_prefix, o_abs_upper, o_abs_lower,
                        o_cert_converged, o_cert_not_converged, o_cert_state);
                    $fatal;
                end
                $display("PASS tb_online_row_update_delta_slice");
                $finish;
            end
        end

        if (!seen_valid) begin
            $display("ERROR tb_online_row_update_delta_slice no valid output");
            $fatal;
        end
    end
endmodule
