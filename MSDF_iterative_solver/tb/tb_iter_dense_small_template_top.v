...
module tb_iter_dense_small_template_top;
    ...
    localparam integer PAYLOAD_WIDTH = VALID_WIDTH + SRC_WIDTH + 2 * COEFF_TERMS_WIDTH + 2 * BIAS_VEC_WIDTH;
    ...
    wire [NUM_CLUSTERS * PAYLOAD_WIDTH - 1 : 0] o_template_words_clusters;
    ...
    iter_dense_small_template_top #(
        ...
        .template_mem_init("MSDF_iterative_solver/generated/blockdiag8_fixed4_templates.memh")
    ) dut (
        .i_clk(i_clk),
        .i_rst(i_rst),
    ...
    initial begin
        ...
        if (o_sched_row_active_clusters !== 8'b1111_1111) begin
            $display("ERROR template top row-active mask mismatch");
            $fatal;
        end
        ...
        if (!o_iter_done || o_seen_mask !== 2'b11) begin
            $display("ERROR template top iteration did not finish");
            $fatal;
        end

        $display("PASS tb_iter_dense_small_template_top");
        $finish;
    end
endmodule
