...
// Multi-row solver-native digit-stream checkpoint.
//
// Two row engines emit online-delayed signed-digit traces in parallel.  Commit
// adapters keep the state width fixed by skipping the leading online latency
// digits and writing the next DATA_WIDTH digits into one shared digit-stream
// state bank.  After commit, replay reconstructs both row values and compares
// them against full-digit bridge references.

module tb_iter_solver_native_multirow_state_commit_replay;
    ...
    initial begin
        ...
        // term0 replays each row's own committed state.
        ...
        for (di = 0; di < DATA_WIDTH; di = di + 1) begin
            ...
            state_digit_p_terms_rows[0*DEGREE + 0] = row0_state0_p[bit_sel];
            state_digit_n_terms_rows[0*DEGREE + 0] = row0_state0_n[bit_sel];
            state_digit_p_terms_rows[0*DEGREE + 1] = row0_state1_p[bit_sel];
            state_digit_n_terms_rows[0*DEGREE + 1] = 1'b0;
            state_digit_p_terms_rows[0*DEGREE + 2] = 1'b0;
            state_digit_n_terms_rows[0*DEGREE + 2] = row0_state2_n[bit_sel];
            state_digit_p_terms_rows[0*DEGREE + 3] = row0_state3_p[bit_sel];
            state_digit_n_terms_rows[0*DEGREE + 3] = 1'b0;

            state_digit_p_terms_rows[1*DEGREE + 0] = 1'b0;
            state_digit_n_terms_rows[1*DEGREE + 0] = row1_state0_n[bit_sel];
            state_digit_p_terms_rows[1*DEGREE + 1] = row1_state1_p[bit_sel];
            state_digit_n_terms_rows[1*DEGREE + 1] = 1'b0;
            state_digit_p_terms_rows[1*DEGREE + 2] = row1_state2_p[bit_sel];
            state_digit_n_terms_rows[1*DEGREE + 2] = row1_state2_n[bit_sel];
            state_digit_p_terms_rows[1*DEGREE + 3] = row1_state3_p[bit_sel];
        ...
        wait_count = 0;
        while ((!(full_seen0 && full_seen1) || !(adapter_done_rows[0] && adapter_done_rows[1])) && wait_count < 32) begin
            @(negedge i_clk);
            if (full_valid_rows[0] && !full_seen0) begin
                full_seen0 = 1'b1;
                full_sum_seen0 = full_sum0;
        ...
        if (!full_seen0 || !full_seen1 || !adapter_done_rows[0] || !adapter_done_rows[1]) begin
            $display("ERROR multirow capture full_seen=%0d%0d done=%0d%0d",
        ...
        if (replay_sum0 !== full_sum_seen0 || replay_sum1 !== full_sum_seen1) begin
            $display("ERROR multirow replay row0=%0d exp0=%0d row1=%0d exp1=%0d p=%h n=%h",
                replay_sum0, full_sum_seen0, replay_sum1, full_sum_seen1,
                read_state_p_rows, read_state_n_rows);
            $fatal;
        end

        $display("PASS tb_iter_solver_native_multirow_state_commit_replay row0=%0d row1=%0d p=%h n=%h",
