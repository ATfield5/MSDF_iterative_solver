...
module tb_iter_wavefront_radius1_multistage_cluster;
    ...
    wire [NUM_STAGES * NUM_ROWS - 1 : 0] w_stage_valid_rows;
    wire [NUM_STAGES * NUM_ROWS - 1 : 0] w_stage_digit_p_rows;
    wire [NUM_STAGES * NUM_ROWS - 1 : 0] w_stage_digit_n_rows;
    ...
    iter_wavefront_radius1_multistage_cluster #(
    ...
    generate
        for (gi = 0; gi < NUM_ROWS; gi = gi + 1) begin : gen_ref
            iter_solver_native_row_digit_engine #(
                ...
                .i_coeff_p_terms(ref_coeff_p_terms_rows[gi * DEGREE * BIT_WIDTH +: DEGREE * BIT_WIDTH]),
                .i_coeff_n_terms(ref_coeff_n_terms_rows[gi * DEGREE * BIT_WIDTH +: DEGREE * BIT_WIDTH]),
                .i_bias_p(ref_bias_p_rows[gi * BIAS_WIDTH +: BIAS_WIDTH]),
                .i_bias_n(ref_bias_n_rows[gi * BIAS_WIDTH +: BIAS_WIDTH]),
    ...
    always @(posedge i_clk) begin
        ...
        if (i_rst || i_start) begin
            ...
            if (w_final_valid_rows[0]) begin
                for (ri = 0; ri < NUM_ROWS; ri = ri + 1) begin
                    captured_wavefront_final_p_rows[ri * DATA_WIDTH + wavefront_capture_count] =
                        w_final_digit_p_rows[ri];
                    captured_wavefront_final_n_rows[ri * DATA_WIDTH + wavefront_capture_count] =
    ...
    always @(posedge i_clk) begin
        ...
        if (ref_rst || ref_start) begin
            ...
            if (ref_valid_rows[0]) begin
                for (ri = 0; ri < NUM_ROWS; ri = ri + 1) begin
                    captured_ref_curr_p_rows[ri * DATA_WIDTH + ref_capture_count] =
                        ref_digit_p_rows[ri];
                    captured_ref_curr_n_rows[ri * DATA_WIDTH + ref_capture_count] =
                        ref_digit_n_rows[ri];
                end
    ...
    initial begin
        ...
        wait_count = 0;
        while (!wavefront_done_seen && wait_count < 128) begin
            @(negedge i_clk);
            wait_count = wait_count + 1;
        end

        if (!wavefront_done_seen || wavefront_capture_count !== DATA_WIDTH) begin
            $display("ERROR multistage wavefront lifecycle final_count=%0d done=%0d",
                wavefront_capture_count,
                wavefront_done_seen);
            $fatal;
        end
        if (w_stage_started_before_prev_done !== {(NUM_STAGES - 1){1'b1}}) begin
            $display("ERROR expected every later stage to start before previous done flags=%b",
        ...
        for (stage = 0; stage < NUM_STAGES; stage = stage + 1) begin
            ...
            if (ref_capture_count !== DATA_WIDTH) begin
                $display("ERROR fullwait reference stage=%0d count=%0d",
                    stage,
                    ref_capture_count);
                $fatal;
            end
        ...
        if (captured_wavefront_final_p_rows !== captured_ref_prev_p_rows ||
            ...
            $display("ERROR multistage wavefront/reference mismatch wf=%h/%h ref=%h/%h",
        ...
        if (wavefront_done_cycle >= fullwait_done_cycle) begin
            $display("ERROR expected multistage wavefront faster wavefront=%0d fullwait=%0d",
                wavefront_done_cycle,
                fullwait_done_cycle);
            $fatal;
        end

        $display("PASS tb_iter_wavefront_radius1_multistage_cluster");
        $display("INFO stages=%0d delay=%0d wavefront_done_cycle=%0d fullwait_model_cycle=%0d saved_cycles=%0d",
            NUM_STAGES,
