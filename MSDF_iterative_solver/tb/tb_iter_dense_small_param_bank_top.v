...
module tb_iter_dense_small_param_bank_top;
    ...
    iter_dense_small_param_bank_top #(
        ...
        .cert_param_mem_init("MSDF_iterative_solver/generated/blockdiag8_cert_params.memh")
    ) dut (
        .i_clk(i_clk),
        .i_rst(i_rst),
    ...
    initial begin
        ...
        if (o_sched_row_active_clusters !== 8'b1111_1111) begin
            $display("ERROR param bank top row-active mask mismatch");
            $fatal;
        end
        ...
        if (!o_iter_done || o_seen_mask !== 2'b11) begin
            $display("ERROR param bank top iteration did not finish");
            $fatal;
        end

        $display("PASS tb_iter_dense_small_param_bank_top");
        $finish;
    end
endmodule
