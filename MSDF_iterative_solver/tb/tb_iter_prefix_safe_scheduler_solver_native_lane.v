...
module tb_iter_prefix_safe_scheduler_solver_native_lane;
    ...
    task check_done_record;
        ...
        begin
            ...
            if (!r_done_mask[row] ||
                ...
                $display("ERROR lane done row=%0d mask=%b digit=%0d/%0d lead=%0d/%0d",
                    row,
                    r_done_mask,
                    got_digit,
                    exp_digit,
    ...
    always @(posedge i_clk) begin
        if (i_rst || i_start) begin
            ...
            if (w_x_valid && (w_x_new_digit_p || w_x_new_digit_n)) begin
                $display("ERROR expected zero row-lane output digit p/n=%0d/%0d",
                    w_x_new_digit_p,
                    w_x_new_digit_n);
                $fatal;
            end
    ...
    initial begin
        ...
        while (w_done_count < NUM_ROWS) begin
            @(posedge i_clk);
            #1;
            if (r_cycle_count > 32'd160) begin
                $display("ERROR lane timeout accept=%0d done=%0d out=%0d pending=%b lane_busy=%0d",
                    w_accept_count,
                    w_done_count,
        ...
        if (w_accept_count !== 32'd4 ||
            ...
            $display("ERROR lane summary accept=%0d done=%0d dispatch=%0d out=%0d pending=%b maxlead=%0d all=%0d dup=%0d",
                w_accept_count,
                w_done_count,
                w_dispatch_count,
                w_output_digit_count,
        ...
        $display("PASS tb_iter_prefix_safe_scheduler_solver_native_lane");
        $display("INFO accept=%0d done=%0d output_digits=%0d maxlead=%0d",
            w_accept_count,
            w_done_count,
            w_output_digit_count,
            w_max_lead_cycles);
