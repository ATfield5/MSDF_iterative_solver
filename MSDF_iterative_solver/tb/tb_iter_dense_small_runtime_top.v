...
module tb_iter_dense_small_runtime_top;
    ...
    task cfg_write_cluster;
        input [CLUSTER_ADDR_WIDTH - 1 : 0] cluster_idx;
        begin
            @(negedge i_clk);
            i_cfg_cluster_addr <= cluster_idx;
    ...
    task load_window;
        ...
        begin
            ...
            if (!o_window_valid || o_window_busy) begin
                $display("ERROR runtime top window load did not finish");
                $fatal;
            end
        end
    endtask
    ...
    integer ci;
    initial begin
        $readmemh("MSDF_iterative_solver/generated/rtl_vectors/blockdiag8/templates.memh", template_mem);
        $readmemh("MSDF_iterative_solver/generated/rtl_vectors/blockdiag8/cert_params.memh", cert_mem);
        ...
        cfg_write_cluster(1'b0);
        cfg_write_cluster(1'b1);
        load_window();
        cfg_load_state(1'b0, 2'd2, 11'd21, 11'd1);
        cfg_load_state(1'b1, 2'd1, 11'd7, 11'd0);

        @(posedge i_clk);
        #1;
        if (o_template_words_clusters !== {template_mem[1], template_mem[0]} ||
            o_cert_param_words_clusters !== {cert_mem[1], cert_mem[0]}) begin
            $display("ERROR runtime top cfg bank load mismatch");
            $fatal;
        end
        if (o_x_old_p_rows_clusters[3 * DATA_WIDTH - 1 -: DATA_WIDTH] !== 11'd21 ||
            o_x_old_n_rows_clusters[3 * DATA_WIDTH - 1 -: DATA_WIDTH] !== 11'd1 ||
            o_x_old_p_rows_clusters[(NUM_ROWS + 2) * DATA_WIDTH - 1 -: DATA_WIDTH] !== 11'd7) begin
            $display("ERROR runtime top state load mismatch");
            $fatal;
        end
        if (o_sched_row_active_clusters !== {NUM_CLUSTERS * NUM_ROWS{1'b1}}) begin
            $display("ERROR runtime top row-active mask mismatch");
            $fatal;
        end
        if (o_cfg_template_write_count !== 32'd2 ||
            o_cfg_cert_write_count !== 32'd2 ||
            o_cfg_state_write_count !== 32'd2 ||
            o_window_load_count !== 32'd1 ||
            o_window_busy_cycles === 32'd0 ||
            o_window_ready_cycles === 32'd0) begin
            $display("ERROR runtime top loader counters tmpl=%0d cert=%0d state=%0d load=%0d busy=%0d ready=%0d",
        ...
        launch_once();
        cycles_waited = 0;
        while (!o_iter_done && cycles_waited < 20) begin
            @(posedge i_clk);
            cycles_waited = cycles_waited + 1;
        end

        if (!o_iter_done || o_seen_mask !== {NUM_CLUSTERS{1'b1}}) begin
            $display("ERROR runtime top iteration did not finish");
            $fatal;
        end
        ...
        for (ci = 0; ci < NUM_CLUSTERS; ci = ci + 1) begin
            if (o_cluster_max_error[(ci + 1) * ACC_WIDTH - 1 -: ACC_WIDTH]
                ...
                $display("ERROR runtime top cluster %0d max_error got=%0d expected=%0d",
                    ci,
            ...
            if (o_cluster_certified[ci] !== 1'b0) begin
                $display("ERROR runtime top cluster %0d certified mismatch", ci);
                $fatal;
            end
        end
        if (!o_iter_continue || o_iter_converged) begin
            $display("ERROR runtime top iteration decision mismatch");
            $fatal;
        end
        @(posedge i_clk);
        #1;
        if (o_iter_count !== 32'd1 ||
            ...
            o_converged_iter !== 32'd0) begin
            $display("ERROR runtime top counters iter=%0d issue=%0d cert_wait=%0d conv_iter=%0d total=%0d",
                o_iter_count, o_issue_cycles, o_cert_wait_cycles, o_converged_iter, o_total_cycles);
            $fatal;
        end

        $display("PASS tb_iter_dense_small_runtime_top");
        $finish;
    end
endmodule
