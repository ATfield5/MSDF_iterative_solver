...
// Minimal dense closed-loop top:
//
//   cluster-specialized row-update/delta/cert datapath
//       -> cluster certification vector
//       -> simple iteration controller
//
// This is still a control/datapath closure shell, not a full solver top.
// It proves that the first solver-level loop is closed:
//
//   start iteration
//       -> cluster-local affine update + delta/cert
//       -> cluster result collection
//       -> done / converged / continue pulse

module iter_dense_small_closed_loop_top #(
    ...
    input      [num_clusters * num_rows * (bit_width + 3) - 1 : 0] i_x_old_p_rows_clusters,
    input      [num_clusters * num_rows * (bit_width + 3) - 1 : 0] i_x_old_n_rows_clusters,
    ...
    generate
        for (gi = 0; gi < num_clusters; gi = gi + 1) begin : gen_clusters
            wire [num_rows - 1 : 0] w_valid_rows_unused;
            wire [num_rows * (bit_width + 3) - 1 : 0] w_sum_p_rows_unused;
            wire [num_rows * (bit_width + 3) - 1 : 0] w_sum_n_rows_unused;
            ...
            online_row_cluster_delta_cert #(
                ...
                .i_x_old_p_rows(i_x_old_p_rows_clusters[(gi + 1) * num_rows * (bit_width + 3) - 1 -: num_rows * (bit_width + 3)]),
                .i_x_old_n_rows(i_x_old_n_rows_clusters[(gi + 1) * num_rows * (bit_width + 3) - 1 -: num_rows * (bit_width + 3)]),
                ...
                .o_valid_rows(w_valid_rows_unused),
                .o_sum_p_rows(w_sum_p_rows_unused),
                .o_sum_n_rows(w_sum_n_rows_unused),
